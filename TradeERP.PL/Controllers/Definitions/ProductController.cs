...
namespace TradeERP.PL.Controllers.Definitions
{
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString, int pageNo = 1)
        {
            try
            {
                var result = await _services.GetPagedProducts(pageNo, searchString);
                return View(result);
            }
            catch (Exception ex)
            {
        ...
        {
            var newCode = await _services.GetNewProductCodeAsync();
        ...
        public async Task<IActionResult> Update(ProductViewModel model)
        {
            ...
            var result = await _services.UpdateProduct(model);
            if (!result.Success)
        ...
        private async Task PopulateData(ProductViewModel model)
        {
            model.Categories = await _lookupService.CategoryLookupAsync();
        }
    }
}
