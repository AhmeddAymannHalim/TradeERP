...
namespace TradeERP.PL.Controllers.Definitions
{
    public class BillDetailsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString, int pageNo = 1)
        {
            try
            {
                var result = await _services.GetPagedBillDetails(pageNo, searchString);
                return View(result);
            }
            catch (Exception ex)
            {
        ...
        {
            var model = new BillDetailsViewModel();
        ...
        public async Task<IActionResult> Create(BillDetailsViewModel model)
        {
            ...
            var result = await _services.AddBillDetails(model);
            if (!result.Success)
        ...
        {
            var model = await _services.GetBillDetailsById(id);
            if (model == null)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Update(BillDetailsViewModel model)
        {
            if (!ModelState.IsValid)
                return Json(new { success = false, errors = ModelState.ToErrorDictionary(), message = _localizer["ValidationError"].Value });

            var result = await _services.UpdateBillDetails(model);
            if (!result.Success)
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                var result = await _services.DeleteBillDetails(id);
                if (result.Success)
        ...
        private async Task PopulateData(BillDetailsViewModel model)
        {
            model.BillMasters = await _lookupService.BillMasterLookupAsync();
