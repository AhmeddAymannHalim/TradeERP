...
namespace TradeERP.PL.Controllers.Definitions
{
    public class SupplierController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString, int pageNo = 1)
        {
            try
            {
                var result = await _services.GetPagedSuppliers(pageNo, searchString);
                return View(result);
            }
            catch (Exception ex)
            {
        ...
        {
            var newCode = await _services.GetNewSupplierCodeAsync();
        ...
        public async Task<IActionResult> Create(SupplierViewModel model)
        {
            ...
            var result = await _services.AddSupplier(model);
            if (!result.Success)
        ...
        {
            var model = await _services.GetSupplierById(id);
            if (model == null)
            {
        ...
        public async Task<IActionResult> Update(SupplierViewModel model)
        {
            ...
            var result = await _services.UpdateSupplier(model);
            if (!result.Success)
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                var result = await _services.DeleteSupplier(id);
                if (result.Success)
