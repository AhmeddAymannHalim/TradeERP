...
namespace TradeERP.PL.Controllers.Definitions
{
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString, int pageNo = 1)
        {
            try
            {
                var result = await _services.GetPagedCategories(pageNo, searchString);
                return View(result);
            }
            catch (Exception ex)
            {
        ...
        {
            var newCode = await _services.GetNewCategoryCodeAsync();
        ...
        public async Task<IActionResult> Create(CategoryViewModel model)
        {
            ...
            var result = await _services.AddCategory(model);
            if (!result.Success)
        ...
        {
            var model = await _services.GetCategoryById(id);
            if (model == null)
            {
        ...
        public async Task<IActionResult> Update(CategoryViewModel model)
        {
            ...
            var result = await _services.UpdateCategory(model);
            if (!result.Success)
